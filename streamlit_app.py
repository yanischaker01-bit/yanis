...
@st.cache_data(ttl=3600, show_spinner=False)  # TTL 1h car données peu fréquentes
def load_vigicrues():
    ...
    results = []
    # Parcours direct des entités de type tronçon (TypEntVigiCru = 5 ou autre)
    for ent in root.get("ListEntVigiCru", []):
        code = ent.get("CdEntVigiCru")
        typ = ent.get("TypEntVigiCru", "5")
        if not code:
            continue
        try:
            detail = get_json(VIGICRUES_URL, params={
                "CdEntVigiCru": code,
                "TypEntVigiCru": typ
            }, headers=headers)
        except Exception:
        ...
        # Recherche des tronçons de rivière dans la réponse
        troncons = detail.get("ListTroncons") or detail.get("ListeTroncons") or []
        for troncon in troncons:
            name = troncon.get("LbTroncon") or troncon.get("NomTroncon") or troncon.get("NomCoursDeau")
            raw_level = troncon.get("NivVigiCru") or troncon.get("NiveauVigilance") or troncon.get("CdCouleur")
            level = {
    ...
    # Dédoublonnage
    unique = list({(x["name"], x["level"]): x for x in results}.values())
    return unique, True
...
with st.sidebar:
    ...
    station_limit = st.slider("Stations à tester", 10, 60, 20, 10)  # réduit à 20 par défaut
...
if module == "Vue rapide":
    ...
    quick = sectors[["name","pk_start","pk_end","communes","static_score","static_level"]].rename(columns={
        "name":"Secteur","pk_start":"PK début","pk_end":"PK fin","communes":"Communes",
        "static_score":"Indice disponible","static_level":"Niveau"
    })
...
# ===================== ALERTES ET PRÉVISIONS =====================
    ...
    with a:
        ...
        if not mf_ok:
            st.warning("Non vérifiée")
        elif not mf_alerts:
            st.success("Aucune vigilance non verte")
        else:
            for x in mf_alerts:
                st.write(f"{x['level']} | Dép. {x['dep']} | {x['phenomenon']} | {x['day']}")
    with b:
    ...
    with c:
        ...
        elif not [x for x in vc_alerts if x["level"] != "VERT"]:
            st.success("Aucune vigilance non verte")
        else:
            for x in sorted(vc_alerts, key=lambda z: -LEVEL_RANK[z["level"]])[:15]:
...
# ===================== PLUIE HISTORIQUE =====================
    ...
    chosen_names = st.multiselect("Communes", names, default=names[:min(3, len(names))])  # 3 par défaut
    ...
    if rain.empty:
        ...
        st.dataframe(maxima.rename(columns={"commune": "Commune", "date": "Date du maximum", "rain_mm": "Maximum (mm)"}),
...
# ===================== PIÉZOMÈTRES ACTIFS =====================
    ...
    if stations.empty:
        ...
        display = stations[[c for c in cols if c in stations]].rename(columns={
            ...
            "distance_km": "Distance LGV (km)", "status": "Emission", "last_date": "Dernière mesure",
            "level": "Niveau NGF", "delta_24h": "Variation 24 h", "delta_7d": "Variation 7 j"
        })
...
# ===================== RISQUE HISTORIQUE =====================
    ...
    # Sélection des communes (limite à 3 par défaut pour réduire les appels)
    coords = selected_points.groupby("commune_name")[["latitude", "longitude"]].mean().reset_index()
    names = sorted(coords["commune_name"].tolist())
